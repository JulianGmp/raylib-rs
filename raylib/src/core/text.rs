...
#[repr(transparent)]
#[derive(Debug)]
pub struct RSliceCharInfo(pub(crate) std::mem::ManuallyDrop<std::boxed::Box<[GlyphInfo]>>);
...
impl std::convert::AsRef<Box<[GlyphInfo]>> for RSliceCharInfo {
    fn as_ref(&self) -> &Box<[GlyphInfo]> {
        &self.0
    }
}

impl std::convert::AsMut<Box<[GlyphInfo]>> for RSliceCharInfo {
